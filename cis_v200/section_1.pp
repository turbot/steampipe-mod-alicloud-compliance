...
control "cis_v200_1_13" {
  title         = "1.13 Ensure RAM password policy expires passwords in 365 days or greater"
  description   = "RAM password policies can require passwords to be expired after a given number of days. It is recommended that the password policy expire passwords in 365 days or greater."
  query         = query.ram_password_policy_expire_365_or_greater
...
control "cis_v200_1_14" {
  ...
  description   = "RAM password policies can enforce temporary lockout after repeated failed attempts. It is recommended that the password policy block logon after 5 incorrect logon attempts within an hour."
...
control "cis_v200_1_15" {
  title         = "1.15 Ensure RAM policies that allow full \"*:*\" administrative privileges are not created"
  description   = "RAM policies that allow all actions on all resources provide full administrative privileges and pose a high security risk. Policies should grant only the permissions required."
  query         = query.ram_policy_no_full_wildcard_privileges
